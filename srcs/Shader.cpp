...
GLuint Shader::LoadShaders(const char * vertex_file_path,const char * fragment_file_path){
	...
	if(VertexShaderStream.is_open()){
		...
		printf("[Debug::Shader] Impossible to open %s. Are you in the right directory ? Don't forget to read the FAQ !\n", vertex_file_path);
		getchar();
		return 0;
	}
	...
	printf("[Debug::Shader] Compiling shader : %s\n", vertex_file_path);
	...
	// Link the program
	printf("[Debug::Shader] Linking program\n");
...
void Shader::updateMatrices(glm::mat4 MVP,glm::mat4 M,glm::mat4 V){
	
	glUniformMatrix4fv(m_MVPID, 1, GL_FALSE, &MVP[0][0]);
	glUniformMatrix4fv(m_VID, 1, GL_FALSE, &M[0][0]);
	glUniformMatrix4fv(m_MID, 1, GL_FALSE, &V[0][0]);
