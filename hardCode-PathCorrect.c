...
#define SMALL_TILE_DISTANCE 0.57 // Need to define. Based on 10cm tile
...
void encoded_lpivot(float revs, long pow){
  ...
  reset_mencoder();
  setMotorSyncEncoder(motorC, motorB, 100, revs_to_degs, pow);
  while(getMotorEncoder(motorB) > -revs_to_degs) {
  	displayCenteredTextLine(2, "left %f", getMotorEncoder(motorB));
  	displayCenteredTextLine(4, "cur %f", revs_to_degs);
...
void pathCorrect(char desiredTile){
	...
	bool attempted_left, attempted_right = false;

	while(true){
		// Look for color pivoting left
		if(left_pivots <= max_pivots && (!attempted_left)){
			turnLeft(REV_360/pivot_increment, rotations, DEFAULT_SPD);
  		short current_colour = SensorValue[Colour];
  		left_pivots++;
  		if(desiredTile=='w' && current_colour > white_threshold){ // Testing for White and white Found
  		forward(SMALL_TILE_DISTANCE/5, rotations, DEFAULT_SPD);
				turnRight(REV_360/pivot_increment * left_pivots, rotations, DEFAULT_SPD);
				reset_mencoder()
				break;
  		}
		...
		// Max Left pivots reached. reset back to normal
			...
			turnRight(REV_360/pivot_increment * max_pivots, rotations,  DEFAULT_SPD);
			attempted_left = true;
		}

		else if(right_pivots <= max_pivots && (!attempted_right)){
			turnRight(REV_360/pivot_increment, rotations, DEFAULT_SPD);
  		short current_colour = SensorValue[Colour];
  		right_pivots++;
  		if(desiredTile=='w' && current_colour > white_threshold){ // Testing for White and white Found
  			forward(SMALL_TILE_DISTANCE/5, rotations, DEFAULT_SPD);
				turnLeft(REV_360/pivot_increment * right_pivots, rotations, DEFAULT_SPD);
				reset_mencoder()
				break;
  		}
  		else if(desiredTile=='b' && current_colour < black_threshold){ // testing for Black and black found
...
void on_track(){
	int black_count = 0;
	int tile_count = 1;
	bool on_black, on_white = false;

	while(tile_count <= 30){
		//encoded_mforward(SMALL_TILE_DISTANCE, DEFAULT_SPD);
		forward(SMALL_TILE_DISTANCE, rotations, DEFAULT_SPD);
		short current_colour = SensorValue[Colour];	/* Reflected Colour */


		if( (tile_count % 2 == 1) || (tile_count == 1) ){ // Expecting White
...
task main(){
	/* Initial Stage */
	encoded_mforward(0.67, DEFAULT_SPD);
  //encoded_rpivot(REV_90, DEFAULT_SPD);
	turnRight(REV_90 * 360 - OFFSET, degrees, DEFAULT_SPD);
  /* After Initial Pivot */
  on_track();
