...
void CMDParser::addOpt(std::string opt, int numValues, std::string optlong, std::string help){
...
void CMDParser::init(int& argc, char** argv){
...
int CMDParser::isOptSet(std::string opt){
  if(_opts[opt] != 0)
    return 1;
  return 0;
}
...
std::vector<int> CMDParser::getOptsInt(std::string opt) {
  std::vector<int> opts;
  if(isOptSet(opt))
...
std::vector<float> CMDParser::getOptsFloat(std::string opt) {
...
std::vector<std::string> CMDParser::getOptsString(std::string opt) {
