...
#include "AutoLight.cginc"
	...
	fs_in vert(vs_in v) {
		...
		//float4 atten = LIGHT_ATTENUATION(v);

		//float3 diffuseReflection = _LightColor0* max(0.0, dot(normalDirection, lightDirection));

		vout.normal = normalDirection;
		vout.colour = v.colour;
		//vout.colour = v.colour * (float4(diffuseReflection, 1.0) + UNITY_LIGHTMODEL_AMBIENT * atten);
	...
	void geo(point fs_in voxelPoint[1], inout TriangleStream<fs_in> stream) {
		...
		v1.colour = colour;
		//v.vertex = mul(_World2Object, v1.pos);
		...
		v2.colour = colour;
		//v.vertex = mul(_World2Object, v2.pos);
		...
		v3.colour = colour;
		//v.vertex = mul(_World2Object, v3.pos);
		...
		v4.colour = colour;
		//v.vertex = mul(_World2Object, v4.pos);
		...
		v5.colour = colour;
		//v.vertex = mul(_World2Object, v5.pos);
		...
		v6.colour = colour;
		//v.vertex = mul(_World2Object, v6.pos);
		...
		v7.colour = colour;
		//v.vertex = mul(_World2Object, v7.pos);
		...
		v8.colour = colour;
		//v.vertex = mul(_World2Object, v8.pos);
